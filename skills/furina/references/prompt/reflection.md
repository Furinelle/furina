...
`type=boundary` 必须使用 `priority=3`。用户明确说“不喜欢”“不要”“别再”“不要催”等边界时，优先记录边界本身，而不是把它降级成普通偏好。
...
9. **灵魂状态类型**：`soul_state` 输出字符串 `low` / `calm` / `active` / `excited`；旧版整数只作为运行时兼容输入，不要主动输出整数（规范化规则见 `../memory/memory_format.md`）。
